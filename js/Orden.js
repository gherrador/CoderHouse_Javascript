    ...
    try {
        $.each(Orden, (index, producto) => {
            ...
            if ($(producto).attr('Cantidad') >= 1) {
                $('#template-finalizarcompra').contents().find('#producto-nombre').text(producto.Nombre)
                $('#template-finalizarcompra').contents().find('#producto-descripcion').text(producto.Descripcion)
                $('#template-finalizarcompra').contents().find('#fotoProductoOrden').attr('src', producto.Foto)
                $('#template-finalizarcompra').contents().find('#producto-cantidad').text(producto.Cantidad)
                $('#template-finalizarcompra').contents().find('#producto-precio').text(`${divisa}` + producto.Precio)
                const cloneCarrito = $('#template-finalizarcompra').contents().clone();
                $(fragment).append(cloneCarrito)
            }
        });
        $('#productos-orden').append(fragment)
        precioTotal()
    } catch (err) {
